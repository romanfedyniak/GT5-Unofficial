...
public class GT_NEI_DefaultHandler extends TemplateRecipeHandler {
    ...
    public void drawExtras(int aRecipeIndex) {
        ...
        if (recipeDesc == null) {
            ...
            if (tDuration > 0) {
                // drawText(10, 113, trans("158","Time: ") + (tDuration < 20 ? "< 1" : Integer.valueOf(tDuration / 20))
                // + trans("161"," secs"), -16777216);
                drawText(
                    10,
                    113,
                    String.format("%s%.2f%s", trans("158", "Time: "), 0.05 * tDuration, trans("161", " secs")),
                    -16777216);
            }
            ...
            if (tSpecial == -100 && GT_Mod.gregtechproxy.mLowGravProcessing) {
                ...
                || (GT_Utility.isStringValid(this.mRecipeMap.mNEISpecialValuePost))) {
                    drawText(
                        10,
                        123,
                        this.mRecipeMap.mNEISpecialValuePre + tSpecial * this.mRecipeMap.mNEISpecialValueMultiplier
    ...
    public static class GT_RectHandler implements IContainerInputHandler, IContainerTooltipHandler {
        ...
        public boolean canHandle(GuiContainer gui) {
            return (gui instanceof GT_GUIContainer_BasicMachine
                ...
                || (gui instanceof GT_GUIContainer_FusionReactor
                    && GT_Utility.isStringValid(((GT_GUIContainer_FusionReactor) gui).mNEI))
                || (gui instanceof GT_GUIContainer_PrimitiveBlastFurnace
                    && GT_Utility.isStringValid(((GT_GUIContainer_PrimitiveBlastFurnace) gui).mNEI));
        }

        public List<String> handleTooltip(GuiContainer gui, int mousex, int mousey, List<String> currenttip) {
            if ((canHandle(gui)) && (currenttip.isEmpty())) {
                if (gui instanceof GT_GUIContainer_BasicMachine && new Rectangle(65, 13, 36, 18).contains(
                    new Point(
                ...
                } else if (gui instanceof GT_GUIContainer_FusionReactor && new Rectangle(145, 0, 24, 24).contains(
                    new Point(
                        GuiDraw.getMousePosition().x - ((GT_GUIContainer_FusionReactor) gui).getLeft()
                            - codechicken.nei.recipe.RecipeInfo.getGuiOffset(gui)[0],
                        GuiDraw.getMousePosition().y - ((GT_GUIContainer_FusionReactor) gui).getTop()
                            - codechicken.nei.recipe.RecipeInfo.getGuiOffset(gui)[1]))) {
                                currenttip.add("Recipes");
                            } else
                    if (gui instanceof GT_GUIContainer_PrimitiveBlastFurnace && new Rectangle(51, 10, 24, 24).contains(
                        new Point(
                            GuiDraw.getMousePosition().x - ((GT_GUIContainer_PrimitiveBlastFurnace) gui).getLeft()
                                - codechicken.nei.recipe.RecipeInfo.getGuiOffset(gui)[0],
                            GuiDraw.getMousePosition().y - ((GT_GUIContainer_PrimitiveBlastFurnace) gui).getTop()
                                - codechicken.nei.recipe.RecipeInfo.getGuiOffset(gui)[1]))) {
                                    currenttip.add("Recipes");
                                }
        ...
        private boolean transferRect(GuiContainer gui, boolean usage) {
            if (gui instanceof GT_GUIContainer_BasicMachine) {
                ...
                return (canHandle(gui))
                    && (new Rectangle(145, 0, 24, 24).contains(
                        new Point(
                            ...
                            GuiDraw.getMousePosition().y - ((GT_GUIContainer_FusionReactor) gui).getTop()
                                - codechicken.nei.recipe.RecipeInfo.getGuiOffset(gui)[1])))
                    && (usage ? GuiUsageRecipe.openRecipeGui(((GT_GUIContainer_FusionReactor) gui).mNEI, new Object[0])
                        : GuiCraftingRecipe.openRecipeGui(((GT_GUIContainer_FusionReactor) gui).mNEI, new Object[0]));
                ...
                return (canHandle(gui))
                    ...
                    && (usage
                        ? GuiUsageRecipe
                            .openRecipeGui(((GT_GUIContainer_PrimitiveBlastFurnace) gui).mNEI, new Object[0])
                        : GuiCraftingRecipe
                            .openRecipeGui(((GT_GUIContainer_PrimitiveBlastFurnace) gui).mNEI, new Object[0]));
            }
            return false;
    ...
    public class CachedDefaultRecipe extends TemplateRecipeHandler.CachedRecipe {
        ...
        public CachedDefaultRecipe(GT_Recipe aRecipe) {
            ...
            if ((aRecipe.mFluidInputs.length > 0) && (aRecipe.mFluidInputs[0] != null)
                ...
                this.mInputs.add(
                    new FixedPositionedStack(GT_Utility.getFluidDisplayStack(aRecipe.mFluidInputs[0], true), 48, 52));
