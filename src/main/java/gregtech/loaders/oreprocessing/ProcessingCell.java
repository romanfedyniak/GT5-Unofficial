            ...
            case cell:
                if (aMaterial == Materials.Empty) {
                    ...
                    if ((aMaterial.mMaterialList.size() > 0) && ((aMaterial.mExtraData & 0x3) != 0)) {
                        ...
                        long tItemAmount = 0L;
                        long tCapsuleCount = GT_ModHandler.getCapsuleCellContainerCountMultipliedWithStackSize(
                        ...
                        for (MaterialStack tMat : aMaterial.mMaterialList) {
                            if (tMat.mAmount > 0L) {
                                ...
                                if (tMat.mMaterial == Materials.Air) {
                                    tStack = ItemList.Cell_Air
                                        .get(tMat.mAmount * tDensityMultiplier / 2L, new Object[0]);
                                } else {
                                ...
                                if (tItemAmount + tMat.mAmount * 3628800L
                                    ...
                                    if (tStack != null) {
                                        ...
                                        if ((tStack.stackSize > 0) && tCapsuleCount
                        ...
                        if (tList.size() > 0) {
                            if ((aMaterial.mExtraData & 0x1) != 0) {
                                // GT_Values.RA.addElectrolyzerRecipe(GT_Utility.copyAmount(tItemAmount, new Object[] {
                                // aStack }), tCapsuleCount > 0L ? (int)tCapsuleCount : 0, (ItemStack)tList.get(0),
                                // tList.size() < 2 ? null : (ItemStack)tList.get(1), tList.size() < 3 ? null :
                                // (ItemStack)tList.get(2), tList.size() < 4 ? null : (ItemStack)tList.get(3),
                                // tList.size() < 5 ? null : (ItemStack)tList.get(4), tList.size() < 6 ? null :
                                // tCapsuleCount < 0L ? ItemList.Cell_Empty.get(-tCapsuleCount, new Object[0]) :
                                // (ItemStack)tList.get(5), (int)Math.max(1L, Math.abs(aMaterial.getProtons() * 8L *
                                // tItemAmount)), Math.min(4, tList.size()) * 30);
                                GT_Values.RA.addElectrolyzerRecipe(
                                    GT_Utility.copyAmount(tItemAmount, new Object[] { aStack }),
                                    tCapsuleCount <= 0L ? 0 : (int) tCapsuleCount,
                                    (ItemStack) tList.get(0),
                                    tList.size() >= 2 ? (ItemStack) tList.get(1) : null,
                                    tList.size() >= 3 ? (ItemStack) tList.get(2) : null,
                                    tList.size() >= 4 ? (ItemStack) tList.get(3) : null,
                                    tList.size() >= 5 ? (ItemStack) tList.get(4) : null,
                                    tCapsuleCount >= 0L ? tList.size() >= 6 ? (ItemStack) tList.get(5) : null
                                        : ItemList.Cell_Empty.get(-tCapsuleCount, new Object[0]),
                                    (int) Math.max(1L, Math.abs(aMaterial.getProtons() * 8L * tItemAmount)),
                                    Math.min(4, tList.size()) * 30);
                            }
                            if ((aMaterial.mExtraData & 0x2) != 0) {
                                // GT_Values.RA.addCentrifugeRecipe(GT_Utility.copyAmount(tItemAmount, new Object[] {
                                // aStack }), tCapsuleCount > 0L ? (int)tCapsuleCount : 0, (ItemStack)tList.get(0),
                                // tList.size() < 2 ? null : (ItemStack)tList.get(1), tList.size() < 3 ? null :
                                // (ItemStack)tList.get(2), tList.size() < 4 ? null : (ItemStack)tList.get(3),
                                // tList.size() < 5 ? null : (ItemStack)tList.get(4), tList.size() < 6 ? null :
                                // tCapsuleCount < 0L ? ItemList.Cell_Empty.get(-tCapsuleCount, new Object[0]) :
                                // (ItemStack)tList.get(5), (int)Math.max(1L, Math.abs(aMaterial.getMass() * 2L *
                                // tItemAmount)));
