...
public class ModularArmor_Item extends ItemArmor implements ISpecialArmor {
    ...
    public ISpecialArmor.ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source,
        ...
        if (player != null && armor != null && source != null) {
            try {
                ...
                if (data.mStat.get(StatType.THORNS) > 0.1d && source != DamageSource.fall
                    && source.getSourceOfDamage() != null) {
                    source.getSourceOfDamage()
                        .attackEntityFrom(new DamageSource("Thorns"), data.mStat.get(StatType.THORNS));
                }
    ...
    public void addInformation(ItemStack itemStack, EntityPlayer player, List info, boolean b) {
        ...
        if (data.info != null) info.addAll(data.info);
    }
    ...
    public void damageArmor(EntityLivingBase entity, ItemStack stack, DamageSource source, int damage, int slot) {
        ...
        if (stack.getItemDamage() > stack.getMaxDamage() / 2 && new Random().nextInt(100) < 5) {
            tmp.getSlot(new Random().nextInt(tmp.getSlotCount()))
                .decrStackSize(1);
             ...
             * EntityItem entityitem = new EntityItem(this.worldObj, this.xCoord + d, this.yCoord + d1, this.zCoord +
             * d2, drop);
    ...
    public void onEntityLivingFallEvent(LivingFallEvent event) {
        if (FMLCommonHandler.instance()
            ...
            if (data != null && event != null
                ...
                && event.distance - 3 <= data.mStat.get(StatType.FALLDEFENCE)) {
                event.setCanceled(true);
            } else if (data != null && event != null
                && data.type == 3
                && data.charge >= data.mStat.get(StatType.PISTONEUUSAGE)) {
    ...
    public void onArmorTick(World aWorld, EntityPlayer aPlayer, ItemStack aStack) {
        ...
        try {
            ...
            if (data.type == 0 && aPlayer.getAir() < 100 && data.fluid != null) {
                int air = 0;
                if (data.fluid.getUnlocalizedName()
                    .equals("fluid.oxygen") && data.fluid.amount >= 150) {
                    aPlayer.setAir(aPlayer.getAir() + 150);
                    air = 150;
                } else if (data.fluid.getUnlocalizedName()
                    .equals("fluid.air") && data.fluid.amount >= 500) {
            ...
            if (data.tooltipUpdate == 40
                ...
                && data.type == 0
                && data.fluid != null
                && data.fluid.getUnlocalizedName()
                    .equals("oxygen")
                && data.fluid.amount < data.mStat.get(StatType.TANKCAP)
                ...
                if (stack != null && stack.getItem() instanceof ModularArmor_Item) {
                    ...
                    ContainerModularArmor tmp2 = new ContainerBasicArmor(
                        ...
                        new InventoryArmor(ModularArmor_Item.class, stack));
                    ArmorCalculation
                        .useFluid(tmp2.mInvArmor.parts, -Math.round(data.mStat.get(StatType.ELECTROLYZERPROD)));
                }
            }
            ...
            if (data.isTopItem) {
                ...
                // if(data.helmet!=null&&data.helmet.openGui){data.helmet.openGui=false;aPlayer.openGui(GT_Values.GT,
                // openGuiNr+400, aWorld, (int) aPlayer.posX, (int) aPlayer.posY, (int) aPlayer.posZ);}
                // if(data.chestplate!=null&&data.chestplate.openGui){data.chestplate.openGui=false;aPlayer.openGui(GT_Values.GT,
                // openGuiNr+300, aWorld, (int) aPlayer.posX, (int) aPlayer.posY, (int) aPlayer.posZ);}
                // if(data.leggings!=null&&data.leggings.openGui){data.leggings.openGui=false;aPlayer.openGui(GT_Values.GT,
                // openGuiNr+200, aWorld, (int) aPlayer.posX, (int) aPlayer.posY, (int) aPlayer.posZ);}
                // if(data.boots!=null&&data.boots.openGui){data.boots.openGui=false;aPlayer.openGui(GT_Values.GT,
                // openGuiNr+100, aWorld, (int) aPlayer.posX, (int) aPlayer.posY, (int) aPlayer.posZ);}
                // Night Vision
                if (timer >= 200) {
                    timer = 0;
                    if (data.mStat.get(StatType.PROCESSINGPOWER) > data.mStat.get(StatType.PROCESSINGPOWERUSED)
                        && data.helmet != null
                        && data.helmet.mBStat.get(StatType.NIGHTVISION)
                        && data.charge > 3) {
                ...
                if (data.mStat.get(StatType.MAGNET) > 1) {
                    ...
                    List<EntityItem> items = aPlayer.worldObj.getEntitiesWithinAABB(
                        EntityItem.class,
                ...
                if (data.leggings != null && data.leggings.charge > data.leggings.mStat.get(StatType.MOTOREUUSAGE)
                    ...
                    && jumpticks > 0
                    && (aPlayer.onGround
                        && Math.abs(aPlayer.motionX) + Math.abs(aPlayer.motionZ) > 0.10000000149011612D)) {
                    data.leggings.charge -= data.leggings.mStat.get(StatType.MOTOREUUSAGE);
