...
public class ArmorData {
    ...
    public ArmorData(EntityPlayer player, ItemStack stack, int type, int tier) {
        ...
        this.type = type;
        this.armorTier = tier;
        ContainerModularArmor tmp = new ContainerBasicArmor(
            (EntityPlayer) player,
            new InventoryArmor(ModularArmor_Item.class, stack));
    ...
    public void updateTooltip() {
        ...
        if (maxCharge != 0) {
            ...
            if (type == 2) {
                tagList.add(
                    GT_LanguageManager.getTranslation("Jumpboost") + ": "
                        + mStat.get(StatType.PISTONJUMPBOOST) / 3
                        + "m");
            }
            ...
            if (type == 2 && mStat.get(StatType.MOTPRPOWER) > 0) {
                ...
                tagList.add(
                    GT_LanguageManager.getTranslation("Motor energy usage") + ": "
                        + mStat.get(StatType.MOTOREUUSAGE)
                        + " EU");
                if (maxWeight > 4000) {
            ...
            tagList.add(
                GT_LanguageManager.getTranslation("Processing power ") + " "
                    + mStat.get(StatType.PARTPROCESSING)
                    + " "
                    + GT_LanguageManager.getTranslation(""));
            tagList.add(
                GT_LanguageManager.getTranslation("Processing power used") + ": "
                    + mStat.get(StatType.PARTPROCESSINGUSED)
                    + " "
                    + GT_LanguageManager.getTranslation(""));
            if (type == 0 && mStat.get(StatType.ELECTROLYZERPROD) > 0) {
                tagList.add(
                    GT_LanguageManager.getTranslation("Electrolyzer produces ") + " "
                        + mStat.get(StatType.ELECTROLYZERPROD) / 2
                        + GT_LanguageManager.getTranslation("per second"));
            }
            if (mStat.get(StatType.TANKCAP) > 0) {
                if (fluid != null) {
                    tagList.add(
                        GT_LanguageManager.getTranslation("Tank Capacity") + ": "
                            + fluid.getLocalizedName()
                            + " "
                            + fluid.amount
                            + "L ("
                            + mStat.get(StatType.TANKCAP)
                            + ")");
                } else {
        ...
        tagList.add(
            GT_LanguageManager.getTranslation("Physical Defence") + ": "
                + (Math.round(mStat.get(StatType.PHYSICALDEFENCE) * 1000) / 10.0)
                + "%");
        tagList.add(
            GT_LanguageManager.getTranslation("Projectile Defence") + ": "
                + (Math.round(mStat.get(StatType.PROJECTILEDEFENCE) * 1000) / 10.0)
                + "%");
        tagList.add(
            GT_LanguageManager.getTranslation("Fire Defence") + ": "
                + (Math.round(mStat.get(StatType.FIREDEFENCE) * 1000) / 10.0)
                + "%");
        tagList.add(
            GT_LanguageManager.getTranslation("Magic Defence") + ": "
                + (Math.round(mStat.get(StatType.MAGICDEFENCE) * 1000) / 10.0)
                + "%");
        tagList.add(
            GT_LanguageManager.getTranslation("Explosive Defence") + ": "
                + (Math.round(mStat.get(StatType.EXPLOSIONDEFENCE) * 1000) / 10.0)
                + "%");
        if (mStat.get(StatType.FALLDEFENCE) > 0 && type == 3) {
            tagList.add(
                GT_LanguageManager.getTranslation("Absorbs") + " "
                    + mStat.get(StatType.FALLDEFENCE)
                    + GT_LanguageManager.getTranslation(" m of Fall Defence"));
        }
            ...
            if (mStat.get(StatType.RADIATIONDEFENCE) > 0.01d) {
                tagList.add(
                    GT_LanguageManager.getTranslation("Radiation Defence") + ": "
                        + (Math.round(mStat.get(StatType.RADIATIONDEFENCE) * 1000) / 10.0)
                        + "%");
            }
        }
        info = tagList;
    ...
    public void armorPartsEquipped(EntityPlayer aPlayer) {
        ...
        if (helmet != null && chestplate != null && leggings != null && boots != null) {
            ...
            set(
                mBStat,
                StatType.FULLRADIATIONARMOR,
                helmHasFullRadiationDefence && chestplateHasFullRadiationDefence
                    && leggingsHasFullRadiationDefence
                    && bootsHasFullRadiationDefence);
            set(
                mBStat,
                StatType.FULLELECTRICARMOR,
                helmHasFullElectricalDefenceDefence && chestplateHasFullElectricalDefenceDefence
                    && leggingsHasFullElectricalDefenceDefence
                    && bootsHasFullElectricalDefenceDefence);
        } else {
    ...
    private void updateArmorStats(ArmorData armorData, ArrayList<StatType> statTypes) {
        for (StatType statType : statTypes) {
            if (armorData == null || armorData.mStat == null || !armorData.mStat.containsKey(statType)) continue;
