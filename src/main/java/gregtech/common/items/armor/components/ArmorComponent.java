...
public abstract class ArmorComponent implements IArmorComponent {
    ...
    public ArmorComponent(String aName, String aOreDict, boolean aElectric, float aWeight) {
        try {
            ...
            for (ItemStack tStack : OreDictionary.getOres(aOreDict))
                if (tStack != null) mStacks.put(tStack.getUnlocalizedName(), this);
    ...
    public ArmorComponent(String aName, ItemStack aStack, boolean aElectric, float aWeight) {
        try {
            mConfigName = aName;
            String tStackName = GregTech_API.sModularArmor.get(
                mConfigName,
                "Stack",
                GameRegistry.findUniqueIdentifierFor(aStack.getItem())
                    .toString() + (aStack.getItemDamage() == 0 ? "" : ":" + aStack.getItemDamage()));
    ...
    public boolean isArmorComponent(ItemStack aStack) {
        if (mStack != null && GT_Utility.areStacksEqual(mStack, aStack, true)) {
            return true;
        }
        if (mOreDict != null) {
