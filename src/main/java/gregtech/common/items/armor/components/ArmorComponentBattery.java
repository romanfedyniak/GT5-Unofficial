...
public class ArmorComponentBattery extends ArmorComponent {
    ...
    public ArmorComponentBattery(String aName, ItemStack aStack, boolean aElectric, float aWeight,
        float aBatteryCapacity) {
        super(aName, aStack, aElectric, aWeight);
        mStat.put(
            StatType.BATTERYCAPACITY,
            (float) GregTech_API.sModularArmor.get(mConfigName, "ProcessingUsed", aBatteryCapacity));
    }
