...
public class GuiElectricArmor1 extends GuiContainer {
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
        ...
        this.mc.getTextureManager()
            .bindTexture(new ResourceLocation("gregtech", "textures/gui/armorgui3x4.png"));
        int xStart = (width - xSize) / 2;
        int yStart = (height - ySize) / 2;
        // Draw background
        ...
        float tankCap = cont.mInvArmor.data.mStat.containsKey(StatType.TANKCAP)
            ? cont.mInvArmor.data.mStat.get(StatType.TANKCAP)
            : 0.0f;
        if (tankCap > 0) {
            drawTexturedModalRect(xStart + 94, yStart + 32, 231, 69, 16, 34);
        }
        float weight = cont.mInvArmor.data.mStat.containsKey(StatType.WEIGHT)
            ? cont.mInvArmor.data.mStat.get(StatType.WEIGHT)
            : 0.0f;
        ...
        if (tab == 0) {
            ...
            bar = Math.min(
                (int) Math.floor(
                    52 * ((float) cont.mInvArmor.data.mStat.get(StatType.PROCESSINGPOWERUSED)
                        / (float) cont.mInvArmor.data.mStat.get(StatType.PROCESSINGPOWER))),
                52);
            ...
            if (cont.mInvArmor.data.mStat.get(StatType.PROJECTILEDEFENCE) > 0)
                drawTexturedModalRect(xStart + 30, yStart + 29, 186, 42, 7, 7);
            drawBars(31, 29, cont.mInvArmor.data.mStat.get(StatType.PROJECTILEDEFENCE));
            if (cont.mInvArmor.data.mStat.get(StatType.FIREDEFENCE) > 0)
                drawTexturedModalRect(xStart + 30, yStart + 38, 186, 51, 7, 7);
            drawBars(31, 38, cont.mInvArmor.data.mStat.get(StatType.FIREDEFENCE));
            if (cont.mInvArmor.data.mStat.get(StatType.MAGICDEFENCE) > 0)
                drawTexturedModalRect(xStart + 30, yStart + 47, 186, 60, 7, 7);
            drawBars(31, 47, cont.mInvArmor.data.mStat.get(StatType.MAGICDEFENCE));
            if (cont.mInvArmor.data.mStat.get(StatType.EXPLOSIONDEFENCE) > 0)
                drawTexturedModalRect(xStart + 30, yStart + 56, 186, 69, 7, 7);
            drawBars(31, 56, cont.mInvArmor.data.mStat.get(StatType.EXPLOSIONDEFENCE));
            if (cont.mInvArmor.data.mStat.get(StatType.RADIATIONDEFENCE) > 0)
                drawTexturedModalRect(xStart + 61, yStart + 20, 186, 87, 7, 7);
            drawBars(62, 20, cont.mInvArmor.data.mStat.get(StatType.RADIATIONDEFENCE));
            if (cont.mInvArmor.data.mStat.get(StatType.ELECTRICALDEFENCE) > 0)
                drawTexturedModalRect(xStart + 61, yStart + 29, 186, 96, 7, 7);
            drawBars(62, 29, cont.mInvArmor.data.mStat.get(StatType.ELECTRICALDEFENCE));
            if (cont.mInvArmor.data.mStat.get(StatType.WITHERDEFENCE) > 0)
                drawTexturedModalRect(xStart + 61, yStart + 38, 186, 105, 7, 7);
            drawBars(62, 38, cont.mInvArmor.data.mStat.get(StatType.WITHERDEFENCE));
            if (cont.mInvArmor.data.mStat.get(StatType.FALLDEFENCE) > 0)
                drawTexturedModalRect(xStart + 61, yStart + 47, 186, 114, 7, 7);
            if (cont.mInvArmor.data.mStat.get(StatType.THORNS) > 0)
                drawTexturedModalRect(xStart + 61, yStart + 56, 186, 123, 7, 7);
            if (cont.mInvArmor.data.mStat.get(StatType.MAGNET) > 0)
                drawTexturedModalRect(xStart + 70, yStart + 56, 186, 78, 7, 7);
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseBtn) {
        ...
        if (yStart > 68 && yStart < 77) {
            ...
            if (xStart > 72 && xStart < 112) {
                if (xStart > 72 && xStart < 81 && cont.mInvArmor.data.helmet != null) {
                    cont.mInvArmor.data.helmet.openGui = true;
                }
                if (xStart > 82 && xStart < 91 && cont.mInvArmor.data.chestplate != null) {
                    cont.mInvArmor.data.chestplate.openGui = true;
                }
                if (xStart > 92 && xStart < 101 && cont.mInvArmor.data.leggings != null) {
                    cont.mInvArmor.data.leggings.openGui = true;
                }
        ...
        // }
        //
        // //GT_Values.NW.sendToServer(new FluidSync(player.getCommandSenderName(), tmp2.amount,
        // GT_Utility.getFluidName(tmp2, false)));
    ...
    protected void drawTooltip(int x, int y) {
        ...
        if (x >= 7 && y >= 11 && x <= 33 && y <= 17) {
            list.add(
                GT_LanguageManager.getTranslation("Weight") + ": " + cont.mInvArmor.data.mStat.get(StatType.WEIGHT));
            list.add("Total Weight: " + cont.mInvArmor.data.maxWeight);
            if (cont.mInvArmor.data.mStat.get(StatType.WEIGHT) > 1000) list.add("Too Heavy!");
        }
        ...
        if (tab == 0) {
            if (x >= 93 && y >= 36 && x <= 110 && y <= 71) {
                list.add("Tank Capacity: " + cont.mInvArmor.data.mStat.get(StatType.TANKCAP) + "L");
            }
            if (x >= 7 && y >= 22 && x <= 70 && y <= 28) {
                list.add("Processing Power Provided: " + cont.mInvArmor.data.mStat.get(StatType.PROCESSINGPOWER));
            ...
            if (x >= 28 && x <= 58) {
                if (y >= 25 && y <= 32) {
                    list.add(
                        GT_LanguageManager.getTranslation("Physical Defence") + ": "
                            + (Math.round(cont.mInvArmor.data.mStat.get(StatType.PHYSICALDEFENCE) * 1000) / 10.0)
                            + "%");
                } else if (y >= 33 && y <= 41) {
                    list.add(
                        GT_LanguageManager.getTranslation("Projectile Defence") + ": "
                            + (Math.round(cont.mInvArmor.data.mStat.get(StatType.PROJECTILEDEFENCE) * 1000) / 10.0)
                            + "%");
                } else if (y >= 42 && y <= 50) {
                    list.add(
                        GT_LanguageManager.getTranslation("Fire Defence") + ": "
                            + (Math.round(cont.mInvArmor.data.mStat.get(StatType.FIREDEFENCE) * 1000) / 10.0)
                            + "%");
                } else if (y >= 51 && y <= 59) {
                    list.add(
                        GT_LanguageManager.getTranslation("Magical Defence") + ": "
                            + (Math.round(cont.mInvArmor.data.mStat.get(StatType.MAGICDEFENCE) * 1000) / 10.0)
                            + "%");
                } else if (y >= 60 && y <= 68) {
                    list.add(
                        GT_LanguageManager.getTranslation("Explosion Defence") + ": "
                            + (Math.round(cont.mInvArmor.data.mStat.get(StatType.EXPLOSIONDEFENCE) * 1000) / 10.0)
                            + "%");
                }
            } else if (x >= 59 && x <= 90) {
                if (y >= 25 && y <= 32) {
                    list.add(
                        GT_LanguageManager.getTranslation("Radiation Defence") + ": "
                            + (Math.round(cont.mInvArmor.data.mStat.get(StatType.RADIATIONDEFENCE) * 1000) / 10.0)
                            + "%");
                    ...
                    list.add(
                        GT_LanguageManager.getTranslation("Electrical Defence") + ": "
                            + (Math.round(cont.mInvArmor.data.mStat.get(StatType.ELECTRICALDEFENCE) * 1000) / 10.0)
                            + "%");
                    ...
                    list.add(
                        GT_LanguageManager.getTranslation("Wither Defence") + ": "
                            + (Math.round(cont.mInvArmor.data.mStat.get(StatType.WITHERDEFENCE) * 1000) / 10.0)
                            + "%");
                } else if (y >= 51 && y <= 59) {
                    if (cont.mInvArmor.data.type != 3) {
                        ...
                        list.add(
                            GT_LanguageManager.getTranslation("Absorbs") + " "
                                + (int) Math.round(cont.mInvArmor.data.mStat.get(StatType.FALLDEFENCE))
                                + GT_LanguageManager.getTranslation("m of Fall Damage"));
                    }
                } else if (y >= 60 && y <= 68) {
                    if (x < 69) {
                        list.add(
                            GT_LanguageManager.getTranslation("Thorns") + ": "
                                + (int) Math.round(cont.mInvArmor.data.mStat.get(StatType.THORNSSINGLE))
                                + " Dmg");
                        list.add(
                            GT_LanguageManager.getTranslation("Total Thorns") + ": "
                                + (int) Math.round(cont.mInvArmor.data.mStat.get(StatType.THORNS))
                                + " Dmg");
                    } else {
                        list.add(
                            GT_LanguageManager.getTranslation("Magnet") + ": "
                                + cont.mInvArmor.data.mStat.get(StatType.MAGNETSINGLE)
                                + " m");
                        list.add(
                            GT_LanguageManager.getTranslation("Total Magnet") + ": "
                                + cont.mInvArmor.data.mStat.get(StatType.MAGNET)
                                + " m");
                    }
                }
            }
        }
        if (!list.isEmpty()) drawHoveringText(list, x, y, fontRendererObj);
    }
