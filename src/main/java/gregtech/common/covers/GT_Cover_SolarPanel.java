    ...
    public int doCoverThings(byte aSide, byte aInputRedstone, int aCoverID, int aCoverVariable, ICoverable aTileEntity,
        ...
        if (aTimer % 100L == 0L) {
            if ((aSide != 1) || (aTileEntity.getWorld()
                ...
                boolean bRain = (aTileEntity.getWorld()
                    .isRaining()) && (aTileEntity.getBiome().rainfall > 0.0F);
                aCoverVariable = bRain && aTileEntity.getWorld().skylightSubtracted >= 4
                    || !aTileEntity.getSkyAtSide(aSide) ? 0
                        : ((int) (!bRain && aTileEntity.getWorld()
                            .isDaytime() ? 1 : 2));
            }
        }
