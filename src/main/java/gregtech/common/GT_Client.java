...
public class GT_Client extends GT_Proxy implements Runnable {
    ...
    static {
        ROTATABLE_VANILLA_BLOCKS = Arrays.asList(
            new Block[] { Blocks.piston, Blocks.sticky_piston, Blocks.furnace, Blocks.lit_furnace, Blocks.dropper,
                Blocks.dispenser, Blocks.chest, Blocks.trapped_chest, Blocks.ender_chest, Blocks.hopper, Blocks.pumpkin,
                Blocks.lit_pumpkin });
    }
    ...
    private static void drawGrid(DrawBlockHighlightEvent aEvent, boolean showCoverConnections) {
        ...
        GL11.glVertex3d(-.25D, .0D, -.50D);
        GL11.glVertex3d(-.25D, .0D, +.50D);
        TileEntity tile = aEvent.player.worldObj
        ...
        if (tile instanceof ICoverable) {
            if (showCoverConnections) {
                for (byte i = 0; i < 6; i++) {
                    if (((ICoverable) tile).getCoverIDAtSide(i) > 0) connections = (byte) (connections + (1 << i));
                }
            } else if (tile instanceof BaseMetaPipeEntity) {
                connections = ((BaseMetaPipeEntity) tile).mConnections;
            }
        }

        if (connections > 0) {
            int[][] GridSwitchArr = new int[][] { { 0, 5, 3, 1, 2, 4 }, { 5, 0, 1, 3, 2, 4 }, { 1, 3, 0, 5, 2, 4 },
    ...
    public void manipulateDensity(EntityViewRenderEvent.FogDensity event) {
        if (GT_Pollution.mPlayerPollution > (GT_Mod.gregtechproxy.mPollutionSmogLimit)) {
            event.density = (0.15f * (Math
                .min(GT_Pollution.mPlayerPollution / ((float) GT_Mod.gregtechproxy.mPollutionSourRainLimit), 1.0f)))
                + 0.1f;
    ...
    public void onPreLoad() {
        super.onPreLoad();
        String arr$[] = { "renadi", "hanakocz", "MysteryDump", "Flaver4", "x_Fame", "Peluche321", "Goshen_Ithilien",
            "manf", "Bimgo", "leagris", "IAmMinecrafter02", "Cerous", "Devilin_Pixy", "Bkarlsson87", "BadAlchemy",
            "CaballoCraft", "melanclock", "Resursator", "demanzke", "AndrewAmmerlaan", "Deathlycraft", "Jirajha",
            "Axlegear", "kei_kouma", "Dracion", "dungi", "Dorfschwein", "Zero Tw0", "mattiagraz85", "sebastiank30",
            "Plem", "invultri", "grillo126", "malcanteth", "Malevolence_", "Nicholas_Manuel", "Sirbab", "kehaan",
            "bpgames123", "semig0d", "9000bowser", "Sovereignty89", "Kris1432", "xander_cage_", "samuraijp", "bsaa",
            "SpwnX", "tworf", "Kadah", "kanni", "Stute", "Hegik", "Onlyme", "t3hero", "Hotchi", "jagoly", "Nullav",
            "BH5432", "Sibmer", "inceee", "foxxx0", "Hartok", "TMSama", "Shlnen", "Carsso", "zessirb", "meep310",
            "Seldron", "yttr1um", "hohounk", "freebug", "Sylphio", "jmarler", "Saberawr", "r00teniy", "Neonbeta",
            "yinscape", "voooon24", "Quintine", "peach774", "lepthymo", "bildeman", "Kremnari", "Aerosalo", "OndraSter",
            "oscares91", "mr10movie", "Daxx367x2", "EGERTRONx", "aka13_404", "Abouttabs", "Johnstaal", "djshiny99",
            "megatronp", "DZCreeper", "Kane_Hart", "Truculent", "vidplace7", "simon6689", "MomoNasty", "UnknownXLV",
            "goreacraft", "Fluttermine", "Daddy_Cecil", "MrMaleficus", "TigersFangs", "cublikefoot", "chainman564",
            "NikitaBuker", "Misha999777", "25FiveDetail", "AntiCivilBoy", "michaelbrady", "xXxIceFirexXx",
            "Speedynutty68", "GarretSidzaka", "HallowCharm977", "mastermind1919", "The_Hypersonic", "diamondguy2798",
    ...
    public void onPostLoad() {
        super.onPostLoad();
        try {
            label0: for (int i = 1; i < GregTech_API.METATILEENTITIES.length; i++) do {
                if (i >= GregTech_API.METATILEENTITIES.length) continue label0;
                if (GregTech_API.METATILEENTITIES[i] != null) GregTech_API.METATILEENTITIES[i].getStackForm(1L)
        ...
        // try {
        // for (; i < GregTech_API.METATILEENTITIES.length; i++) if (GregTech_API.METATILEENTITIES[i] != null)
    ...
    public void run() {
         ...
         * Scanner tScanner = new Scanner(new
         * URL("http://files.minecraftforge.net/maven/com/gregoriust/gregtech/message.txt").openStream());
    ...
    public void onPlayerTickEventClient(TickEvent.PlayerTickEvent aEvent) {
        if ((aEvent.side.isClient()) && (aEvent.phase == TickEvent.Phase.END) && (!aEvent.player.isDead)) {
                ...
                try {
                    for (GT_Recipe recipe : GT_Recipe.GT_Recipe_Map.sAssemblylineVisualRecipes.mRecipeList) {
                        recipe.mHidden = !sfw.hasAchievementUnlocked(
                            GT_Mod.achievements.getAchievement(
                                recipe.getOutput(0)
                                    .getUnlocalizedName()));
                    }
                } catch (Exception e) {}
             ...
             * if ((this.mMessage.length() > 5) && (GregTech_API.sSpecialFile.get(ConfigCategories.news, this.mMessage,
             * true))) {
             * aEvent.player.addChatComponentMessage(new ChatComponentText(this.mMessage));
             * }
             * try {
             * int tVersion = Integer.parseInt(((String)
             * Class.forName("ic2.core.IC2").getField("VERSION").get(null)).substring(4, 7));
             * if (GT_Values.D1) {
             * GT_Log.out.println("Industrialcraft Version: " + tVersion);
             * }
             * if (tVersion < 624) {
             * aEvent.player.addChatComponentMessage(new
             * ChatComponentText("GregTech: Please update your IndustrialCraft here:"));
             * aEvent.player.addChatComponentMessage(new ChatComponentText("ic2api.player.to:8080/job/IC2_experimental/"
             * + (GT_Mod.MAX_IC2 < Integer.MAX_VALUE ? GT_Mod.MAX_IC2 : 624) + "/"));
             * } else if (tVersion > GT_Mod.MAX_IC2) {
             * aEvent.player.addChatComponentMessage(new
             * ChatComponentText("GregTech: Please downgrade your IndustrialCraft here:"));
             * aEvent.player.addChatComponentMessage(new ChatComponentText("ic2api.player.to:8080/job/IC2_experimental/"
             * + GT_Mod.MAX_IC2 + "/"));
             * }
             * } catch (Throwable e) {
             * aEvent.player.addChatComponentMessage(new
             * ChatComponentText("GregTech: Please get the recommended Version of IndustrialCraft here:"));
             * aEvent.player.addChatComponentMessage(new ChatComponentText("ic2api.player.to:8080/job/IC2_experimental/"
             * + (GT_Mod.MAX_IC2 < Integer.MAX_VALUE ? GT_Mod.MAX_IC2 : 624) + "/"));
             * }
             * }
    ...
    public void onDrawBlockHighlight(DrawBlockHighlightEvent event) {
        final TileEntity tileEntity = event.player.worldObj
            .getTileEntity(event.target.blockX, event.target.blockY, event.target.blockZ);
        final Block block = event.player.worldObj
            .getBlock(event.target.blockX, event.target.blockY, event.target.blockZ);

        if (tileEntity == null) return;

        if (GT_Utility.isStackInList(event.currentItem, GregTech_API.sWrenchList)) {
            if (tileEntity instanceof ITurnable || tileEntity instanceof IWrenchable
                || ROTATABLE_VANILLA_BLOCKS.contains(block)) drawGrid(event, false);
            return;
        }

        if (!(tileEntity instanceof ICoverable)) return;

        if (GT_Utility.isStackInList(event.currentItem, GregTech_API.sCrowbarList)) {
            drawGrid(event, true);
            return;
        }

        if (GT_Utility.isStackInList(event.currentItem, GregTech_API.sCovers.keySet())) {
            if (((ICoverable) tileEntity).getCoverIDAtSide((byte) event.target.sideHit) == 0) {
                drawGrid(event, true);
                return;
            }
        }
        ...
        if (tileEntity instanceof BaseMetaPipeEntity) {
            if (GT_Utility.isStackInList(event.currentItem, GregTech_API.sWireCutterList)
                || GT_Utility.isStackInList(event.currentItem, GregTech_API.sSolderingToolList)) drawGrid(event, false);
            else if (GT_Utility.isStackInList(event.currentItem, GregTech_API.sScrewdriverList)) drawGrid(event, true);
            return;
        }

        if (GT_Utility.isStackInList(event.currentItem, GregTech_API.sSolderingToolList)) {
            drawGrid(event, true);
            return;
        }

        if (tileEntity instanceof IEnergyConnected
            && (GT_Utility.isStackInList(event.currentItem, GregTech_API.sWireCutterList)
                || GT_Utility.isStackInList(event.currentItem, GregTech_API.sSolderingToolList))) {
            IEnergyConnected energyConnectedTileEntity = (IEnergyConnected) tileEntity;
            for (byte i = 0; i < 6; i++) {
                if (energyConnectedTileEntity.inputEnergyFrom(i, false)
                    || energyConnectedTileEntity.outputsEnergyTo(i, false)) {
                    drawGrid(event, false);
                    return;
                }
            }
        }
        ...
        if (event.currentItem == null && event.player.isSneaking()) {
            if (((ICoverable) tileEntity).getCoverIDAtSide((byte) event.target.sideHit) == 0) {
                for (byte i = 0; i < 6; i++) {
                    if (((ICoverable) tileEntity).getCoverIDAtSide(i) > 0) {
                        drawGrid(event, true);
                        return;
                    }
                }
    ...
    public void doSonictronSound(ItemStack aStack, World aWorld, double aX, double aY, double aZ) {
        ...
        if (tString.startsWith("random.explode")) if (aStack.stackSize == 3) tString = "random.fuse";
        else if (aStack.stackSize == 2) tString = "random.old_explode";
        if (tString.startsWith("streaming.")) switch (aStack.stackSize) {
            case 1: // '\001'
                tString = (new StringBuilder()).append(tString)
                    .append("13")
                    .toString();
                break;
