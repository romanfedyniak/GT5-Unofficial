...
public class GT_UndergroundOil {
    ...
    public static FluidStack undergroundOil(Chunk chunk, float readOrDrainCoefficient) {
        ...
        final XSTR tRandom = new XSTR(
            (aWorld.getSeed() + aWorld.provider.dimensionId * 2
        ...
        GT_UO_Fluid uoFluid = GT_Mod.gregtechproxy.mUndergroundOil.GetDimension(aWorld.provider.dimensionId)
            .getRandomFluid(tRandom);
        ...
        if (uoFluid == null || uoFluid.getFluid() == null) {
            ...
            if (tInts[GTOILFLUID] == uoFluid.getFluid()
                .getID()) {// if stored fluid matches uoFluid
                ...
                fluidInChunk.amount = (int) ((float) fluidInChunk.amount * (0.75f + (random.nextFloat() / 2f)));// Randomly
                                                                                                                // change
                                                                                                                // amounts
                                                                                                                // by
                                                                                                                // +/-
        ...
        if (readOrDrainCoefficient >= 0) {
            ...
            if (fluidExtracted <= 0 || fluidInChunk.amount <= decrease) {// decrease - here it is max value of
                                                                         // extraction for easy check
                fluidInChunk = null;
                tInts[GTOIL] = 0;// so in next access it will stop way above
            } else {
                fluidInChunk.amount = fluidExtracted;// give appropriate amount
                if (random.nextFloat() < (decrease - averageDecrease)) decrease--;// use random to "subtract double from
                                                                                  // int"
                // ex.
                // averageDecrease=3.9
