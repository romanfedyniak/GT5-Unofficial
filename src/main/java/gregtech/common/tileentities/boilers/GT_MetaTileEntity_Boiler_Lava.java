...
public class GT_MetaTileEntity_Boiler_Lava extends GT_MetaTileEntity_Boiler {
    ...
    public GT_MetaTileEntity_Boiler_Lava(int aID, String aName, String aNameRegional) {
        super(
            ...
            aName,
            aNameRegional,
            new String[] { "A Boiler running off Lava", "Produces 600L of Steam per second",
                "Drops to 200L of Steam per second as byproduct slot fills",
                "Clean out byproducts to keep efficiency high", "Causes 20 Pollution per second" });
    }
        ...
        if ((aBaseMetaTileEntity.isServerSide()) && (aTick > 20L)) {
                    ...
                    if ((this.mFluid == null) || (!GT_ModHandler.isWater(this.mFluid)) || (this.mFluid.amount <= 0)) {
                        ...
                        if (GT_Mod.gregtechproxy.mSmallLavaBoilerEfficiencyLoss) {
                            ...
                            if (byproductStack != null && !(GT_Utility.isStackInvalid(byproductStack))) {
                                ...
                                efficiency = 1.0
                                    - (double) byproductStack.stackSize / (double) byproductStack.getMaxStackSize();
                            }
                        }

                        // Decrease water amount in proportion to steam production
                        // Can't decrease by a fraction, as fluid amounts are integers, so need to decrease randomly so
                        // expected amount consumed is correct
            ...
            if ((this.mProcessingEnergy <= 0) && (aBaseMetaTileEntity.isAllowedToWork())
                && (GT_OreDictUnificator
                    .isItemStackInstanceOf(this.mInventory[2], OrePrefixes.bucket.get(Materials.Lava)))) {
                this.mProcessingEnergy += 1000;
                // Either we only had one bucket of lava in the input, and this is fine, or we had a stack of lava
                // buckets.
                // Those can only stack when cheating, so we don't care about voiding them as we can just cheat in more
                aBaseMetaTileEntity
                    .setInventorySlotContents(2, GT_OreDictUnificator.get(OrePrefixes.bucket, Materials.Empty, 1L));
            }
            if ((this.mTemperature < 1000) && (this.mProcessingEnergy > 0) && (aTick % 8L == 0L)) {
                ...
                if (aBaseMetaTileEntity.getRandomNumber(333) == 0) {
                    ...
                    if (!aBaseMetaTileEntity
                        .addStackToSlot(3, GT_OreDictUnificator.get(OrePrefixes.dustImpure, Materials.Stone, 1L))) {
                        // If the output slot had something in it already, stick one dust in:
                        aBaseMetaTileEntity.setInventorySlotContents(
                            3,
                            GT_OreDictUnificator.get(OrePrefixes.dustImpure, Materials.Stone, 1));
                    }
                }
            }
