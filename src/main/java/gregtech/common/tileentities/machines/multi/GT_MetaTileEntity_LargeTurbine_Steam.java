...
public class GT_MetaTileEntity_LargeTurbine_Steam extends GT_MetaTileEntity_LargeTurbine {
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex,
        ...
        return new ITexture[] { Textures.BlockIcons.MACHINE_CASINGS[1][aColorIndex + 1],
            aFacing == aSide
                ? aActive ? new GT_RenderedTexture(Textures.BlockIcons.LARGETURBINE_ST_ACTIVE5)
                    : new GT_RenderedTexture(Textures.BlockIcons.LARGETURBINE_ST5)
                : Textures.BlockIcons.CASING_BLOCKS[57] };
    }

    public String[] getDescription() {
        return new String[] { "Controller Block for the Large Steam Turbine",
            "Size(WxHxD): 3x3x4 (Hollow), Controller (Front centered)", "1x Steam Input Hatch (Side centered)",
            "1x Maintenance Hatch (Side centered)", "1x Dynamo Hatch (Back centered)",
            "Turbine Casings for the rest (24 at least!)", "Needs a Turbine Item (Inside controller GUI)",
    ...
    int fluidIntoPower(ArrayList<FluidStack> aFluids, int aOptFlow, int aBaseEff) {
        ...
        int flow = 0;
        int remainingFlow = (int) (aOptFlow * 1.25f); // Allowed to use up to 125% of optimal flow. Variable required
                                                      // outside of loop for multi-hatch scenarios.
        ...
        for (int i = 0; i < aFluids.size() && remainingFlow > 0; i++) { // loop through each hatch; extract inputs and
            ...
            if (fluidName.equals("fluid.steam") || fluidName.equals("ic2.fluidSteam")
                ...
                flow = Math.min(flow, Math.min(remainingFlow, (int) (aOptFlow * 1.25f))); // try to use up to 125% of
                                                                                          // optimal flow w/o exceeding
                                                                                          // remainingFlow
