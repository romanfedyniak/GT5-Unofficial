...
public abstract class GT_MetaTileEntity_LargeTurbine extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack) {
        ...
        if ((getBaseMetaTileEntity().getAirAtSideAndDistance(getBaseMetaTileEntity().getBackFacing(), 1))
            && (getBaseMetaTileEntity().getAirAtSideAndDistance(getBaseMetaTileEntity().getBackFacing(), 2))) {
            int tAirCount = 0;
            ...
            for (byte i = 2; i < 6; i = (byte) (i + 1)) {
                ...
                if ((null != (tTileEntity = getBaseMetaTileEntity().getIGregTechTileEntityAtSideAndDistance(i, 2)))
                    && (tTileEntity.getFrontFacing() == getBaseMetaTileEntity().getFrontFacing())
    ...
    public boolean checkRecipe(ItemStack aStack) {
        if (aStack == null || !(aStack.getItem() instanceof GT_MetaGenerated_Tool)
            ...
            || aStack.getItemDamage() > 179) return false;
    ...
    public String[] getInfoData() {
        ...
        if (mInventory[1] != null && mInventory[1].getItem() instanceof GT_MetaGenerated_Tool_01) {
            tDura = (int) ((100.0f / GT_MetaGenerated_Tool.getToolMaxDamage(mInventory[1])
                * (GT_MetaGenerated_Tool.getToolDamage(mInventory[1])) + 1));
        }

        return new String[] { "Large Turbine", tRunning, "Current Output: " + mEUt + " EU/t",
            "Optimal Flow: " + (int) realOptFlow + " L/t", "Fuel Remaining: " + storedFluid + "L",
            "Current Speed: " + (mEfficiency / 100) + "%", "Turbine Damage: " + tDura + "%", tMaintainance };
    }
