...
public abstract class GT_MetaTileEntity_OreDrillingPlantBase extends GT_MetaTileEntity_DrillerBase {
    ...
    protected boolean workingDownward(ItemStack aStack, int xDrill, int yDrill, int zDrill, int xPipe, int zPipe,
        ...
        while ((oreBlock == null || oreBlock == Blocks.air) && !oreBlockPositions.isEmpty()) {
            ...
            if (GT_Utility.eraseBlockByFakePlayer(
                ...
                true))
                oreBlock = getBaseMetaTileEntity()
        ...
        if (oreBlock != null && oreBlock != Blocks.air) {
            ArrayList<ItemStack> oreBlockDrops = getBlockDrops(
                oreBlock,
                oreBlockPos.chunkPosX,
                oreBlockPos.chunkPosY,
                oreBlockPos.chunkPosZ);
    ...
    private ItemStack[] getOutputByDrops(ArrayList<ItemStack> oreBlockDrops) {
        ...
        while (!oreBlockDrops.isEmpty()) {
            ItemStack currentItem = oreBlockDrops.remove(0)
                .copy();
    ...
    private void tryAddOreBlockToMineList(int x, int y, int z) {
        ...
        if (block instanceof GT_Block_Ores_Abstract) {
            ...
            if (tTileEntity != null && tTileEntity instanceof GT_TileEntity_Ores
                && ((GT_TileEntity_Ores) tTileEntity).mNatural) oreBlockPositions.add(blockPos);
        } else {
            ...
            if (association != null && association.mPrefix.toString()
                .startsWith("ore")) oreBlockPositions.add(blockPos);
        }
    }
