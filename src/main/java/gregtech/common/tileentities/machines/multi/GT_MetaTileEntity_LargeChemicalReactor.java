...
public class GT_MetaTileEntity_LargeChemicalReactor extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public String[] getDescription() {
        return new String[] { "Controller block for the Large Chemical Reactor",
            "Has the same recipes as the Chemical Reactor", "Does not lose efficiency when overclocked",
            "Accepts fluids instead of fluid cells", "Size(WxHxD): 3x3x3",
            "3x3x3 of Chemically Inert Machine Casings (hollow, min 8!)", "Controller (Front centered)",
            "1x PTFE Pipe Machine Casing (inside the hollow casings)",
            "1x Cupronickel Coil Block (next to PTFE Pipe Machine Casing)", "1x Input Bus/Hatch (Any inert casing)",
            "1x Output Bus/Hatch (Any inert casing)", "1x Maintenance Hatch (Any inert casing)",
        ...
        if (aSide == aFacing) {
            return new ITexture[] { Textures.BlockIcons.casingTexturePages[1][48],
                new GT_RenderedTexture(
                    aActive ? Textures.BlockIcons.OVERLAY_FRONT_LARGE_CHEMICAL_REACTOR_ACTIVE
    ...
    public boolean checkRecipe(ItemStack aStack) {
        ...
        if (inputs.length > 0 || fluids.length > 0) {
            ...
            GT_Recipe recipe = GT_Recipe.GT_Recipe_Map.sMultiblockChemicalRecipes.findRecipe(
        ...
        for (int x = -1 + xDir; x <= xDir + 1; x++) {
            for (int z = -1 + zDir; z <= zDir + 1; z++) {
                for (int y = -1; y <= 1; y++) {
                    ...
                    if (centerCoords == 2 && block == GregTech_API.sBlockCasings5
                        && aBaseMetaTileEntity.getMetaIDOffset(x, y, z) == 0) {
                        hasHeatingCoil = true;
                        continue;
                    }
