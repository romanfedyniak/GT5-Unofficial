...
public class GT_MetaTileEntity_OilCracker extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public String[] getDescription() {
        return new String[] { "Controller Block for the Oil Cracking Unit",
            ...
            "1x Cracked Hydrocarbon Output Hatch (Any left/right side casing)", "1x Maintenance Hatch (Any casing)",
            "1x Energy Hatch (Any casing)", "Clean Stainless Steel Machine Casings for the rest (18 at least!)",
        ...
        if (aSide == aFacing) {
            return new ITexture[] { Textures.BlockIcons.CASING_BLOCKS[49],
                new GT_RenderedTexture(
                    aActive ? Textures.BlockIcons.OVERLAY_FRONT_OIL_CRACKER_ACTIVE
                        : Textures.BlockIcons.OVERLAY_FRONT_OIL_CRACKER) };
        }
        return new ITexture[] { Textures.BlockIcons.CASING_BLOCKS[49] };
    }
    ...
    public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack) {
                ...
                for (int j = -1; j < 2; j++) {// height
                    for (int h = -2; h < 3; h++) {
                        if (!(j == 0 && i == 0 && (h == -1 || h == 0 || h == 1))) {
                            if (h == 1 || h == -1) {
                                if (aBaseMetaTileEntity.getBlockOffset(xDir + i, j, h + zDir)
                            ...
                            if (h == 2 || h == -2) {
                                IGregTechTileEntity tTileEntity = aBaseMetaTileEntity
                                    .getIGregTechTileEntityOffset(xDir + i, j, h + zDir);
                                ...
                                } else if (!addEnergyInputToMachineList(tTileEntity, 49)
                                    && !addMaintenanceToMachineList(tTileEntity, 49)) {
                                        if (aBaseMetaTileEntity.getBlockOffset(xDir + i, j, h + zDir)
                            ...
                            if (h == 0) {
                                IGregTechTileEntity tTileEntity = aBaseMetaTileEntity
                                    .getIGregTechTileEntityOffset(xDir + i, j, h + zDir);
                                if ((!addMaintenanceToMachineList(tTileEntity, 49))
                                    && (!addInputToMachineList(tTileEntity, 49))
            ...
            for (int i = -1; i < 2; i++) {// zDirection
                for (int j = -1; j < 2; j++) {// height
                    for (int h = -2; h < 3; h++) {
                        if (!(j == 0 && i == 0 && (h == -1 || h == 0 || h == 1))) {
                            if (h == 1 || h == -1) {
                                if (aBaseMetaTileEntity.getBlockOffset(xDir + h, j, i + zDir)
                                    != GregTech_API.sBlockCasings5) {
                                    return false;
                                }
                            ...
                            if (h == 2 || h == -2) {
                                IGregTechTileEntity tTileEntity = aBaseMetaTileEntity
                                    .getIGregTechTileEntityOffset(xDir + h, j, i + zDir);
                                if (addInputToMachineList(tTileEntity, 49)) {
                                    if (h == -2) {
                                        negSideInput = true;
                                    } else {
        ...
        if ((negSideInput && negSideOutput) || (posSideInput && posSideOutput)
            || (negSideInput && posSideInput)
            || (negSideOutput && posSideOutput)) {
            return false;
        }
                ...
                for (int zPos = tZ - 1; zPos <= tZ + 1; zPos += (xDir != 0 ? 2 : 1)) {
                    ...
                    if (aBaseMetaTileEntity.getBlock(xPos, yPos, zPos) == GregTech_API.sBlockCasings1
                        ...
                        aBaseMetaTileEntity.getWorld()
                            .setBlock(xPos, yPos, zPos, GregTech_API.sBlockCasings5, 0, 3);
                    }
                }
            }
        }
    ...
    public ArrayList<FluidStack> getStoredFluids() {
        ...
        for (GT_MetaTileEntity_Hatch_Input tHatch : mInputHatches) {
            ...
            if (isValidMetaTileEntity(tHatch) && tHatch.getFillableStack() != null) {
                ...
                if (tStack.isFluidEqual(GT_ModHandler.getSteam(1000))
                    || tStack.isFluidEqual(Materials.Hydrogen.getGas(1000))) {
    ...
    private boolean isHatchInMiddleRing(GT_MetaTileEntity_Hatch_Input inputHatch) {
        if (orientation == ForgeDirection.NORTH || orientation == ForgeDirection.SOUTH) {
            return inputHatch.getBaseMetaTileEntity()
                .getXCoord() == this.controllerX;
        } else {
            return inputHatch.getBaseMetaTileEntity()
                .getZCoord() == this.controllerZ;
        }
    }
}
