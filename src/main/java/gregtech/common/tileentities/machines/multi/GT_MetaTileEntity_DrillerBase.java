...
public abstract class GT_MetaTileEntity_DrillerBase extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex,
        ...
        if (aSide == aFacing) return new ITexture[] { Textures.BlockIcons.CASING_BLOCKS[casingTextureIndex],
    ...
    public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack) {
        ...
        for (int xOff = -1 + back.offsetX; xOff <= 1 + back.offsetX; xOff++) {
            for (int zOff = -1 + back.offsetZ; zOff <= 1 + back.offsetZ; zOff++) {
                ...
                if (!checkCasingBlock(xOff, 0, zOff) && !addMaintenanceToMachineList(tTileEntity, casingTextureIndex)
                    ...
                    && !addDataAccessToMachineList(tTileEntity, casingTextureIndex)) return false;
            }
        }
        ...
        for (int yOff = 1; yOff < 4; yOff++) {
            if (!checkCasingBlock(back.offsetX, yOff, back.offsetZ)
                ...
                || !checkFrameBlock(back.offsetX, yOff, back.offsetZ - 1)
                || !checkFrameBlock(back.offsetX, yOff + 3, back.offsetZ)) return false;
        }
        return true;
    }
