...
public abstract class GT_MetaTileEntity_PrimitiveBlastFurnace extends MetaTileEntity {
    ...
    public boolean allowCoverOnSide(byte aSide, GT_ItemStack aCoverID) {
        return (GregTech_API.getCoverBehavior(aCoverID.toStack())
        ...
        if ((aBaseMetaTileEntity.isClientSide()) && (aBaseMetaTileEntity.isActive())) {
            aBaseMetaTileEntity.getWorld()
                .spawnParticle(
                    ...
                    aBaseMetaTileEntity.getOffsetZ(aBaseMetaTileEntity.getBackFacing(), 1) + (new XSTR()).nextFloat(),
                    0.0D,
                    0.3D,
                    0.0D);
        }
    ...
    private boolean spaceForOutput(ItemStack outputStack, int relativeOutputSlot) {
        ...
        if (((this.mInventory[absoluteSlot].stackSize + outputStack.stackSize
    ...
    private boolean checkRecipe() {
        ...
        GT_Recipe recipe = GT_Recipe.GT_Recipe_Map.sPrimitiveBlastRecipes
            .findRecipe(getBaseMetaTileEntity(), false, 0, null, inputs);
        if (recipe == null) {
            this.mOutputItems = null;
            return false;
        }
