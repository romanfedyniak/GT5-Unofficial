...
public class GT_MetaTileEntity_AdvSeismicProspector extends GT_MetaTileEntity_BasicMachine {
    ...
    public String[] getDescription() {
        return new String[] {
            "Place, activate with explosives (" + "8 Glyceryl, " + "32 TNT or " + "16 ITNT), use Data Stick",
            "Ore prospection area 191x191 blocks", "Oil prospection area 3x3 oilfields" };
    }

    protected GT_MetaTileEntity_AdvSeismicProspector(String aName, int aTier, String aDescription,
        ITexture[][][] aTextures, String aGUIName, String aNEIName, int aNear, int aMiddle, int aRadius, int aStep) {
    ...
    private void prospectOils(ArrayList<String> aOils) {
        ...
        int xChunk = (tChunk.xPosition / range) * range
            - ((tChunk.xPosition < 0 && tChunk.xPosition % range != 0) ? range : 0);
        int zChunk = (tChunk.zPosition / range) * range
            - ((tChunk.zPosition < 0 && tChunk.zPosition % range != 0) ? range : 0);
        ...
        try {
            for (int z = -1; z <= 1; ++z) {
                for (int x = -1; x <= 1; ++x) {
                    ChunkCoordIntPair cInts = getBaseMetaTileEntity().getWorld()
                    ...
                    for (int i = 0; i < range; i++) {
                        for (int j = 0; j < range; j++) {
                            tChunk = getBaseMetaTileEntity().getWorld()
                                .getChunkFromChunkCoords(xChunk + i + x * 6, zChunk + j + z * 6);
    ...
    private void prospectOres(Map<String, Integer> aNearOres, Map<String, Integer> aMiddleOres,
        Map<String, Integer> aFarOres) {
        ...
        for (int i = tLeftXBound; i <= tRightXBound; i += step)
            for (int k = tLeftZBound; k <= tRightZBound; k += step) {
                int di = Math.abs(
                    i - this.getBaseMetaTileEntity()
                        .getXCoord());
                int dk = Math.abs(
                    k - this.getBaseMetaTileEntity()
                        .getZCoord());

                if (di <= near && dk <= near) prospectHole(i, k, aNearOres);
                else if (di <= middle && dk <= middle) prospectHole(i, k, aMiddleOres);
                else prospectHole(i, k, aFarOres);
            }
    }
    ...
    private String checkForOre(int x, int y, int z) {
        ...
        if (tBlock instanceof GT_Block_Ores_Abstract) {
            ...
            if ((tAssotiation != null) && (tAssotiation.mPrefix.toString()
                .startsWith("ore"))) return tAssotiation.mMaterial.mMaterial.mDefaultLocalName;
        }
