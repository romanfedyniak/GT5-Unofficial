...
public class GT_MetaTileEntity_Scanner extends GT_MetaTileEntity_BasicMachine {
    ...
    public GT_MetaTileEntity_Scanner(int aID, String aName, String aNameRegional, int aTier) {
        super(
            ...
            "Scanner.png",
            "",
            new ITexture[] { new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_SIDE_SCANNER_ACTIVE),
                new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_SIDE_SCANNER),
                new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_FRONT_SCANNER_ACTIVE),
                new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_FRONT_SCANNER),
                new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_TOP_SCANNER_ACTIVE),
                new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_TOP_SCANNER),
                new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_BOTTOM_SCANNER_ACTIVE),
        ...
        if (getOutputAt(0) != null) {
            ...
            if (ItemList.Tool_DataOrb.isStackEqual(getSpecialSlot(), false, true)) {
                ...
                if ((tData != null) && ((tData.mPrefix == OrePrefixes.dust) || (tData.mPrefix == OrePrefixes.cell))
                    ...
                    this.mMaxProgresstime = ((int) (tData.mMaterial.mMaterial.getMass() * 8192L
                        / (1 << this.mTier - 1)));
                    this.mEUt = (32 * (1 << this.mTier - 1) * (1 << this.mTier - 1));
                    return 2;
                }
            ...
            if (ItemList.Tool_DataStick.isStackEqual(getSpecialSlot(), false, true)) {
                if (ItemList.Tool_DataStick.isStackEqual(aStack, false, true)) {
                    aStack.stackSize -= 1;
                    this.mOutputItems[0] = GT_Utility
                        .copyAmount(1L, new Object[] { ItemList.Tool_DataStick.get(1, new Object[] {}) });
                        ...
                        for (int i = 0; i < tRecipe.mOreDictAlt.length; i++) {
                            if (tRecipe.mOreDictAlt[i] != null && tRecipe.mOreDictAlt[i].length > 0) {
                                ...
                                for (int j = 0; j < tRecipe.mOreDictAlt[i].length; j++) {
                                    tNBT.setTag(
                                        "a" + i + ":" + j,
                                        tRecipe.mOreDictAlt[i][j].writeToNBT(new NBTTagCompound()));
                                }
                            }
                        }
                        ...
                        for (int i = 0; i < tRecipe.mInputs.length; i++) {
                            if (tRecipe.mOreDictAlt[i] != null) {
                                ...
                                if (FMLCommonHandler.instance()
                                    ...
                                    if (s == null) s = tRecipe.mInputs[i].getDisplayName();
                                }
                                tNBTList.appendTag(
                                    new NBTTagString(
                                        "Input Bus " + (i + 1) + ": " + tRecipe.mInputs[i].stackSize + " " + s));
                            }
                        }
                        for (int i = 0; i < tRecipe.mFluidInputs.length; i++) {
                            if (tRecipe.mFluidInputs[i] != null) {
                                ...
                                if (FMLCommonHandler.instance()
                                    .getEffectiveSide()
                                    .isServer()) {
                                    s = GT_Assemblyline_Server.lServerNames
                                        .get(tRecipe.mFluidInputs[i].getLocalizedName());
                                    if (s == null) s = tRecipe.mFluidInputs[i].getLocalizedName();
                                }
                                tNBTList.appendTag(
                                    new NBTTagString(
                                        "Input Hatch " + (i + 1) + ": " + tRecipe.mFluidInputs[i].amount + "L " + s));
                            }
                        }
