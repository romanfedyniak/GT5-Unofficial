...
public class GT_MetaTileEntity_Pump extends GT_MetaTileEntity_Hatch {
    ...
    public GT_MetaTileEntity_Pump(int aID, String aName, String aNameRegional, int aTier) {
        ...
        super(
            ...
            new String[] { "The best way to empty Oceans!",
                "Pumping Area: " + (GT_MetaTileEntity_Pump.getMaxDistanceForTier((byte) aTier) * 2 + 1)
                    + "x"
        ...
        if (getBaseMetaTileEntity().isServerSide()) {
            ...
            if (this.mPumpCountBelow <= 0) {
                ...
                if ((getBaseMetaTileEntity().isAllowedToWork())
                    && (getBaseMetaTileEntity().isUniversalEnergyStored(this.getEuUsagePerAction()))
                    ...
                    if (!this.hasValidFluid()) {
                        ...
                        // We somehow have a valid fluid, but the head of the pump isn't below the pump. Perhaps someone
                        // broke some pipes
    ...
    private boolean hasValidFluid() {
        return (!GT_Utility.isBlockInvalid(this.mPrimaryPumpedBlock)
            && !GT_Utility.isBlockInvalid(this.mSecondaryPumpedBlock));
    }
    ...
    private boolean consumeFluid(int aX, int aY, int aZ) {
        ...
        if ((GT_Utility.isBlockValid(aBlock))
            && ((this.mPrimaryPumpedBlock == aBlock) || (this.mSecondaryPumpedBlock == aBlock))) {
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex,
        ...
        return new ITexture[] { Textures.BlockIcons.MACHINE_CASINGS[mTier][aColorIndex + 1],
            (aSide == 0 || aSide == 1) ? new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_PIPE_OUT)
    ...
    public ITexture[] getTexturesInactive(ITexture aBaseTexture) {
        return new ITexture[] { new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_ADV_PUMP),
            new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_ADV_PUMP),
            new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_ADV_PUMP),
            new GT_RenderedTexture(Textures.BlockIcons.OVERLAY_ADV_PUMP), };
    }
