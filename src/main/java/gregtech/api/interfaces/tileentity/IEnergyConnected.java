 ...
 * IHasWorldObjectAndCoords is needed for the InWorld related Stuff. @BaseTileEntity does implement most of that
 * Interface.
 */
        ...
        public static final long emitEnergyToNetwork(long aVoltage, long aAmperage, IEnergyConnected aEmitter) {
            ...
            for (byte i = 0, j = 0; i < 6 && aAmperage > rUsedAmperes; i++) if (aEmitter.outputsEnergyTo(i)) {
                ...
                if (tTileEntity instanceof IEnergyConnected) {
                    ...
                    ForgeDirection tDirection = ForgeDirection.getOrientation(i)
                        .getOpposite();
                                                ...
                                                : aExplosionPower < V[4] * 2 ? 6.0F
                                                    : aExplosionPower < V[5] ? 7.0F
                                                        : aExplosionPower < V[6] ? 8.0F
                                                            : aExplosionPower < V[7] ? 9.0F : 10.0F;
                            int tX = tTileEntity.xCoord, tY = tTileEntity.yCoord, tZ = tTileEntity.zCoord;
                            World tWorld = tTileEntity.getWorldObj();
                            GT_Utility
                                .sendSoundToPlayers(tWorld, GregTech_API.sSoundList.get(209), 1.0F, -1, tX, tY, tZ);
                            tWorld.setBlock(tX, tY, tZ, Blocks.air);
                            if (GregTech_API.sMachineExplosions) if (GT_Mod.gregtechproxy.mPollution)
