...
public class GT_MetaPipeEntity_Cable extends MetaPipeEntity implements IMetaTileEntityCable {
    ...
    private void pullFromIc2EnergySources(IGregTechTileEntity aBaseMetaTileEntity, byte[] aSides) {
        ...
        for (byte i = 0; i < aSides.length; i++) {
            ...
            else tEmitter = (tTileEntity == null || tTileEntity instanceof IEnergyTile || EnergyNet.instance == null)
                ? tTileEntity
    ...
    public long transferElectricity(byte aSide, long aVoltage, long aAmperage, HashSet<TileEntity> aAlreadyPassedSet) {
        ...
        if (aVoltage > 0) for (byte i = 0; i < 6 && aAmperage > rUsedAmperes; i++) if (i != aSide
            && isConnectedAtSide(i)
            && baseMetaTile.getCoverBehaviorAtSide(i)
                .letsEnergyOut(i, baseMetaTile.getCoverIDAtSide(i), baseMetaTile.getCoverDataAtSide(i), baseMetaTile)) {
                    ...
                    if (tTileEntity != null && aAlreadyPassedSet.add(tTileEntity)) {
                        ...
                        if (tMeta instanceof IMetaTileEntityCable) {
                            if (tBaseMetaTile.getCoverBehaviorAtSide(tSide)
                                .letsEnergyIn(
                                    tSide,
                                    tBaseMetaTile.getCoverIDAtSide(tSide),
                                    tBaseMetaTile.getCoverDataAtSide(tSide),
                                    tBaseMetaTile)
        ...
        if (GT_Mod.gregtechproxy.mAE2Integration && tTileEntity instanceof appeng.tile.powersink.IC2) {
            if (((appeng.tile.powersink.IC2) tTileEntity).acceptsEnergyFrom((TileEntity) baseMetaTile, tDirection)) {
                ...
                while (aAmperage > rUsedAmperes && ((appeng.tile.powersink.IC2) tTileEntity).getDemandedEnergy() > 0
                    && ((appeng.tile.powersink.IC2) tTileEntity).injectEnergy(tDirection, aVoltage, aVoltage)
                        <= aVoltage)
                    rUsedAmperes++;
            ...
            if (tTileEntity instanceof IEnergyHandlerGC) {
                if (!(tTileEntity instanceof IConnector)
                    ...
                    if (tSizeToReceive >= tStored
                        ...
                        float tReceived = ((IEnergyHandlerGC) tTileEntity)
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        if (aBaseMetaTileEntity.isServerSide()) {
            if (GT_Mod.gregtechproxy.ic2EnergySourceCompat && IC2RectorAtSide[0] >= 0)
                pullFromIc2EnergySources(aBaseMetaTileEntity, IC2RectorAtSide);
    ...
    public boolean onWireCutterRightClick(byte aSide, byte aWrenchingSide, EntityPlayer aPlayer, float aX, float aY,
        ...
        if (GT_Mod.gregtechproxy.gt6Cable
            ...
            if (isConnectedAtSide(aWrenchingSide)) {
                ...
                if (connect(aWrenchingSide) > 0) GT_Utility.sendChatToPlayer(aPlayer, trans("214", "Connected"));
            }
            return true;
        }
        return false;
    ...
    public boolean canConnect(byte aSide, TileEntity tTileEntity) {
        ...
        if (GregTech_API.mGalacticraft) {
            if (tTileEntity instanceof IEnergyHandlerGC && (!(tTileEntity instanceof IConnector)
                || ((IConnector) tTileEntity).canConnect(tDir, NetworkType.POWER))) return true;
        }
        ...
        if (GT_Mod.gregtechproxy.mAE2Integration) {
            if (tTileEntity instanceof IEnergySource && tTileEntity instanceof IPartHost
                ...
                && ((IEnergySource) tTileEntity).emitsEnergyTo((TileEntity) baseMetaTile, tDir)) return true;
        ...
        if (GregTech_API.mInputRF && (tTileEntity instanceof IEnergyEmitter
            && ((IEnergyEmitter) tTileEntity).emitsEnergyTo((TileEntity) baseMetaTile, tDir))) return true;
    ...
    public String[] getDescription() {
        return new String[] {
            "Max Voltage: %%%" + EnumChatFormatting.GREEN
                + mVoltage
                + " ("
                + VN[GT_Utility.getTier(mVoltage)]
                + ")"
