...
public abstract class GT_MetaTileEntity_Hatch extends GT_MetaTileEntity_BasicTank {
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex,
        ...
        int texturePointer = (byte) (actualTexture & 0x7F);// just to be sure, from my testing the 8th bit cannot be set
                                                           // clientside
        return aSide != aFacing
            ? textureIndex > 0 ? new ITexture[] { Textures.BlockIcons.casingTexturePages[mTexturePage][texturePointer] }
                : new ITexture[] { Textures.BlockIcons.MACHINE_CASINGS[mTier][aColorIndex + 1] }
            : textureIndex > 0
                ? aActive ? getTexturesActive(Textures.BlockIcons.casingTexturePages[mTexturePage][texturePointer])
                    : getTexturesInactive(Textures.BlockIcons.casingTexturePages[mTexturePage][texturePointer])
                : aActive ? getTexturesActive(Textures.BlockIcons.MACHINE_CASINGS[mTier][aColorIndex + 1])
    ...
    public void onPreTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {// in that method since it is usually
        ...
        if (actualTexture != mMachineBlock) {// revert to page 0 on edition of the field - old code way
            ...
            mMachineBlock = actualTexture;// clear last bit in mMachineBlock since now we are at page 0 after the direct
                                          // field change
