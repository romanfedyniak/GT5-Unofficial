...
public class GT_MetaPipeEntity_Fluid extends MetaPipeEntity {
    ...
    public void onEntityCollidedWithBlock(World aWorld, int aX, int aY, int aZ, Entity aEntity) {
        if ((((BaseMetaPipeEntity) getBaseMetaTileEntity()).mConnections & -128) == 0
            ...
            for (FluidStack tFluid : mFluids) {
                if (tFluid != null) {
                    int tTemperature = tFluid.getFluid()
    ...
    private void distributeFluid(int index, IGregTechTileEntity aBaseMetaTileEntity) {
        ...
        for (byte aSide, i = 0, j = (byte) aBaseMetaTileEntity.getRandomNumber(6); i < 6; i++) {
            ...
            if (isConnectedAtSide(aSide) && tTank != null
                ...
                && getBaseMetaTileEntity().getCoverBehaviorAtSide(aSide)
                    .letsFluidOut(
                        aSide,
                ...
                && (gTank == null || gTank.getCoverBehaviorAtSide(tSide)
                    .letsFluidIn(
                        tSide,
                        gTank.getCoverIDAtSide(tSide),
                        gTank.getCoverDataAtSide(tSide),
                        tFluid.getFluid(),
                        gTank))) {
        ...
        if (GT_Mod.gregtechproxy.gt6Pipe) {
            ...
            if (aPlayer.isSneaking()) {
                ...
                if (!isConnectedAtSide(tSide)) {
                    if (connect(tSide) > 0) GT_Utility.sendChatToPlayer(aPlayer, trans("214", "Connected"));
                } else {
                    disconnect(tSide);
    ...
    public void doSound(byte aIndex, double aX, double aY, double aZ) {
        ...
        if (aIndex == 9) {
            ...
            for (byte i = 0; i < 6; i++) for (int l = 0; l < 2; ++l) getBaseMetaTileEntity().getWorld()
                .spawnParticle(
                    "largesmoke",
                    aX - 0.5 + (new XSTR()).nextFloat(),
                    aY - 0.5 + (new XSTR()).nextFloat(),
                    aZ - 0.5 + (new XSTR()).nextFloat(),
