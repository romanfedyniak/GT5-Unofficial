...
public abstract class GT_MetaBase_Item extends GT_Generic_Item
            ...
            if (tStats[3] > 0) {
                aList.add(
                    EnumChatFormatting.AQUA + String.format(
                        trans("009", "Contains %s EU   Tier: %s"),
                ...
                if (tStats[3] == -2 && tCharge <= 0) {
                    aList.add(
                        EnumChatFormatting.AQUA + trans("010", "Empty. You should recycle it properly.")
                            + EnumChatFormatting.GRAY);
                } else {
                    aList.add(
                        String.valueOf(EnumChatFormatting.AQUA)
                            + String.format(
                                trans("011", "%s / %s EU - Voltage: %s"),
        ...
        if (tStats != null && tStats[0] > 0) {
            ...
            aList.add(
                EnumChatFormatting.BLUE
                    + ((tFluid == null ? trans("012", "No Fluids Contained") : GT_Utility.getFluidName(tFluid, true)))
                    + EnumChatFormatting.GRAY);
            aList.add(
                EnumChatFormatting.BLUE
                    + String
                        .format(trans("013", "%sL / %sL"), "" + (tFluid == null ? 0 : tFluid.amount), "" + tStats[0])
                    + EnumChatFormatting.GRAY);
        }
     ...
     * }
     * @Override
     * public final int charge(ItemStack aStack, int aCharge, int aTier, boolean aIgnoreTransferLimit, boolean
     * aSimulate) {
     * Long[] tStats = getElectricStats(aStack);
     * if (tStats == null || tStats[2] > aTier || !(tStats[3] == -1 || tStats[3] == -3 || (tStats[3] < 0 && aCharge ==
     * Integer.MAX_VALUE)) || aStack.stackSize != 1) return 0;
     ...
     * (aIgnoreTransferLimit?aCharge:Math.min(tStats[1], aCharge)));
     * if (!aSimulate) setCharge(aStack, tNewCharge);
     * return (int)(tNewCharge-tChargeBefore);
     * }
     * @Override
     * public final int discharge(ItemStack aStack, int aCharge, int aTier, boolean aIgnoreTransferLimit, boolean
     * aSimulate) {
     * Long[] tStats = getElectricStats(aStack);
     * if (tStats == null || tStats[2] > aTier) return 0;
     * if (tStats[3] > 0) {
     ...
     * }
     * long tChargeBefore = getRealCharge(aStack), tNewCharge = Math.max(0, tChargeBefore -
     * (aIgnoreTransferLimit?aCharge:Math.min(tStats[1], aCharge)));
     * if (!aSimulate) setCharge(aStack, tNewCharge);
     ...
     * IElectricItem tArmorItem = (IElectricItem)tArmor.getItem();
     * if (tArmorItem.canProvideEnergy(tArmor) && tArmorItem.getTier(tArmor) >= getTier(aStack)) {
     * int tCharge = ElectricItem.manager.discharge(tArmor, charge(aStack, Integer.MAX_VALUE-1, Integer.MAX_VALUE, true,
     * true), Integer.MAX_VALUE, true, false);
     * if (tCharge > 0) {
     * charge(aStack, tCharge, Integer.MAX_VALUE, true, false);
     * if (aPlayer instanceof EntityPlayer) {
     * Container tContainer = ((EntityPlayer)aPlayer).openContainer;
