...
@Optional.InterfaceList(
    value = { @Optional.Interface(iface = "forestry.api.arboriculture.IToolGrafter", modid = MOD_ID_FR),
...
public abstract class GT_MetaGenerated_Tool extends GT_MetaBase_Item
            ...
            if (name.equals("gt.metatool.01.170") || name.equals("gt.metatool.01.172")
                ...
                aList.add(
                    ...
                    EnumChatFormatting.WHITE + String.format(
                        trans("001", "Durability: %s/%s"),
                        "" + EnumChatFormatting.GREEN + (tMaxDamage - getToolDamage(aStack)) + " ",
                        " " + tMaxDamage) + EnumChatFormatting.GRAY);
                aList.add(
                    tOffset + 1,
                    EnumChatFormatting.WHITE + String.format(
                        trans("002", "%s lvl %s"),
                ...
                aList.add(
                    tOffset + 4,
                    EnumChatFormatting.WHITE + String.format(
                        trans("007", "Optimal Gas flow(EU burnvalue per tick): %sEU/t"),
                        "" + EnumChatFormatting.LIGHT_PURPLE
                            + Math.max(
                                Float.MIN_NORMAL,
                ...
                aList.add(
                    tOffset + 5,
                    EnumChatFormatting.WHITE + String.format(
                        trans("008", "Optimal Plasma flow(Plasma energyvalue per tick): %sEU/t"),
                        "" + EnumChatFormatting.LIGHT_PURPLE
                            + Math.max(
                                Float.MIN_NORMAL,
                                tStats.getSpeedMultiplier() * getPrimaryMaterial(aStack).mToolSpeed * 2000)
                            + EnumChatFormatting.GRAY));
    ...
    public boolean isItemStackUsable(ItemStack aStack) {
        ...
        ConcurrentHashMap<Integer, Integer> tMap = new ConcurrentHashMap<Integer, Integer>(),
            tResult = new ConcurrentHashMap<Integer, Integer>();
