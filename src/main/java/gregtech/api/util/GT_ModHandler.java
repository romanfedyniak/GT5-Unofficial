...
public class GT_ModHandler {
    ...
    private static final List<IRecipe> sAllRecipeList = /* Collections.synchronizedList( */new ArrayList<IRecipe>(
        5000)/* ) */, sBufferRecipeList = new ArrayList<IRecipe>(1000);
    public static volatile int VERSION = 509;
    public static Collection<String> sNativeRecipeClasses = new HashSet<String>(),
        sSpecialRecipeClasses = new HashSet<String>();
    public static GT_HashSet<GT_ItemStack> sNonReplaceableItems = new GT_HashSet<GT_ItemStack>();
    public static Object sBoxableWrapper = GT_Utility
        .callConstructor("gregtechmod.api.util.GT_IBoxableWrapper", 0, null, false);
    ...
    /**
     * Gets an Item from RailCraft, but the Damage Value can be specified, and returns a Replacement Item with the same
     * Damage if not possible
     */
    ...
    public static boolean addSmeltingRecipe(ItemStack aInput, ItemStack aOutput) {
        ...
        FurnaceRecipes.smelting()
            .func_151394_a(aInput, GT_Utility.copy(aOutput), 0.0F);
        return true;
    }
    ...
    public static boolean addLiquidTransposerRecipe(ItemStack aEmptyContainer, FluidStack aLiquid,
        ...
        if (!GT_Mod.gregtechproxy.mTEMachineRecipes
            && !GregTech_API.sRecipeFile.get(ConfigCategories.Machines.liquidtransposer, aFullContainer, true))
            return false;
        try {
    ...
    public static boolean addLiquidTransposerFillRecipe(ItemStack aEmptyContainer, FluidStack aLiquid,
        ...
        if (aEmptyContainer == null || aFullContainer == null || aLiquid == null) return false;
        if (!GT_Mod.gregtechproxy.mTEMachineRecipes
            && !GregTech_API.sRecipeFile.get(ConfigCategories.Machines.liquidtransposerfilling, aFullContainer, true))
            return false;
        try {
    ...
    public static boolean addLiquidTransposerEmptyRecipe(ItemStack aFullContainer, FluidStack aLiquid,
        ...
        if (!GT_Mod.gregtechproxy.mTEMachineRecipes
            && !GregTech_API.sRecipeFile.get(ConfigCategories.Machines.liquidtransposeremptying, aFullContainer, true))
            return false;
        try {
    ...
    public static boolean addRCBlastFurnaceRecipe(ItemStack aInput, ItemStack aOutput, int aTime) {
        ...
        try {
            mods.railcraft.api.crafting.RailcraftCraftingManager.blastFurnace
                .addRecipe(aInput, true, false, aTime, aOutput);
        } catch (Throwable e) {
            return false;
        }
        return true;
    ...
    public static boolean addPulverisationRecipe(ItemStack aInput, ItemStack aOutput1, ItemStack aOutput2, int aChance2,
        ...
        if (GT_Utility.getContainerItem(aInput, false) == null) {
            if (GT_Mod.gregtechproxy.mAddGTRecipesToIC2Machines
                && GregTech_API.sRecipeFile.get(ConfigCategories.Machines.maceration, aInput, true)) {
            ...
            if (!OrePrefixes.log.contains(aInput)) {
                ...
                if (Materials.Wood.contains(aOutput1)) {
                    if (aEnableTEMachineRecipes
                        && GregTech_API.sRecipeFile.get(ConfigCategories.Machines.pulverization, aInput, true)) {
                        if (aOutput2 == null) ThermalExpansion
                            .addSawmillRecipe(32000, GT_Utility.copy(aInput), GT_Utility.copy(aOutput1));
                        else ThermalExpansion.addSawmillRecipe(
                            32000,
                            GT_Utility.copy(aInput),
                            GT_Utility.copy(aOutput1),
                    ...
                    if (aEnableTEMachineRecipes
                        && GregTech_API.sRecipeFile.get(ConfigCategories.Machines.pulverization, aInput, true)) {
                        if (aOutput2 == null) ThermalExpansion
                            .addPulverizerRecipe(32000, GT_Utility.copy(aInput), GT_Utility.copy(aOutput1));
                        else ThermalExpansion.addPulverizerRecipe(
                            32000,
                            GT_Utility.copy(aInput),
                            GT_Utility.copy(aOutput1),
                            GT_Utility.copy(aOutput2),
    ...
    public static boolean addImmersiveEngineeringRecipe(ItemStack aInput, ItemStack aOutput1, ItemStack aOutput2,
        ...
        if (GregTech_API.mImmersiveEngineering && GT_Mod.gregtechproxy.mImmersiveEngineeringRecipes) {
            ...
            try {
                if (mRecipeIE == null) {
                    mRecipeIE = Class.forName("blusunrize.immersiveengineering.common.IERecipes")
                        .getDeclaredMethod(
                            "addCrusherRecipe",
                            ItemStack.class,
                            ItemStack.class,
    ...
    public static boolean addMagneticraftRecipe(ItemStack aInput, ItemStack aOutput1, ItemStack aOutput2, int aChance2,
        ...
        if (GregTech_API.mMagneticraft && GT_Mod.gregtechproxy.mMagneticraftRecipes) {
            ...
            if (tData != null && tData.mPrefix != null) {
                if (tData.mPrefix == OrePrefixes.ore || tData.mPrefix == OrePrefixes.oreBlackgranite
                    || tData.mPrefix == OrePrefixes.oreEndstone
                    || tData.mPrefix == OrePrefixes.oreNetherrack
                    || tData.mPrefix == OrePrefixes.oreRedgranite) {
                    registerMagneticraftCrusherRecipe(
                        aInput,
                        aOutput1,
                        aOutput2,
                        (float) ((float) aChance2 / GT_Mod.gregtechproxy.mMagneticraftBonusOutputPercent),
                        aOutput3,
                        (float) ((float) aChance3 / GT_Mod.gregtechproxy.mMagneticraftBonusOutputPercent));
                } else if (tData.mPrefix == OrePrefixes.crushed || tData.mPrefix == OrePrefixes.crushedCentrifuged
                    || tData.mPrefix == OrePrefixes.crushedPurified) {
                        registerMagneticraftGrinderRecipe(
                            aInput,
                            aOutput1,
                            aOutput2,
                            (float) ((float) aChance2 / GT_Mod.gregtechproxy.mMagneticraftBonusOutputPercent),
    ...
    public static boolean registerMagneticraftSifterRecipe(final ItemStack in, final ItemStack out,
        final ItemStack extra, final float prob) {
        try {
            if (mRecipeMag == null) {
                mRecipeMag = Class.forName("com.cout970.magneticraft.api.access.MgRecipeRegister");
            }
            if (mRecipeMagSifter == null) {
                mRecipeMagSifter = mRecipeMag.getDeclaredMethod(
                    "registerSifterRecipe",
                    ItemStack.class,
                    ItemStack.class,
                    ItemStack.class,
                    float.class);
    ...
    public static boolean registerMagneticraftCrusherRecipe(final ItemStack in, final ItemStack out0,
        final ItemStack out1, final float prob1, final ItemStack out2, final float prob2) {
        try {
            if (mRecipeMag == null) {
                mRecipeMag = Class.forName("com.cout970.magneticraft.api.access.MgRecipeRegister");
            }
            if (mRecipeMagCrusher == null) {
                mRecipeMagCrusher = mRecipeMag.getDeclaredMethod(
            ...
            return (boolean) mRecipeMagCrusher.invoke(
                null,
                in,
                out0,
                out1,
                (float) ((float) prob1 / GT_Mod.gregtechproxy.mMagneticraftBonusOutputPercent),
                out2,
                (float) ((float) prob2 / GT_Mod.gregtechproxy.mMagneticraftBonusOutputPercent));
        } catch (ClassNotFoundException | NoSuchMethodException | SecurityException | IllegalAccessException
            | IllegalArgumentException | InvocationTargetException e) {
    ...
    public static boolean registerMagneticraftGrinderRecipe(final ItemStack in, final ItemStack out0,
        final ItemStack out1, final float prob1, final ItemStack out2, final float prob2) {
        try {
            if (mRecipeMag == null) {
                mRecipeMag = Class.forName("com.cout970.magneticraft.api.access.MgRecipeRegister");
            ...
            if (mRecipeMagGrinder == null) {
                mRecipeMagGrinder = mRecipeMag.getDeclaredMethod(
                    "registerGrinderRecipe",
                    ItemStack.class,
                    ItemStack.class,
                    ItemStack.class,
                    float.class,
            ...
            return (boolean) mRecipeMagGrinder.invoke(
                null,
                in,
                out0,
                out1,
    ...
    public static boolean addSawmillRecipe(ItemStack aInput1, ItemStack aOutput1, ItemStack aOutput2) {
        ...
        if (!GT_Mod.gregtechproxy.mTEMachineRecipes
            && !GregTech_API.sRecipeFile.get(ConfigCategories.Machines.sawmill, aInput1, true)) return false;
        try {
    ...
    public static boolean addInductionSmelterRecipe(ItemStack aInput1, ItemStack aInput2, ItemStack aOutput1,
        ...
        if (!GT_Mod.gregtechproxy.mTEMachineRecipes && !GregTech_API.sRecipeFile
            .get(ConfigCategories.Machines.inductionsmelter, aInput2 == null ? aInput1 : aOutput1, true)) return false;
        try {
            ThermalExpansion.addSmelterRecipe(
                aEnergy * 10,
                GT_Utility.copy(aInput1),
                aInput2 == null ? new ItemStack(Blocks.sand, 1, 0) : aInput2,
                aOutput1,
                aOutput2,
                aChance);
        } catch (Throwable e) {/* Do nothing */}
                ...
                for (ItemStack tStack : ((IRecipeInput) tRecipe.getKey()).getInputs()) {
                    if (GT_Utility.isStackValid(tStack)) {
                        if (aAddGTRecipe
                                    ...
                                    case "gt.recipe.macerator":
                                        aGTRecipeMap.addRecipe(
                                            ...
                                            new ItemStack[] { GT_Utility
                                                .copyAmount(((IRecipeInput) tRecipe.getKey()).getAmount(), tStack) },
                                            (ItemStack[]) ((RecipeOutput) tRecipe.getValue()).items.toArray(),
                                            null,
                                            null,
                                            null,
                                            null,
                            ...
                            // System.out.println("#####Processed IC2 " + aGTRecipeMap.mUnlocalizedName + " Recipe: In("
                            // + tStack.getUnlocalizedName() + ") - Out(" + ((RecipeOutput)
                            // tRecipe.getValue()).items.get(0).getUnlocalizedName() + ")");
                        }
                        if (aRemoveIC2Recipe)
    ...
    public static boolean addRollingMachineRecipe(ItemStack aResult, Object[] aRecipe) {
        ...
        try {
            mods.railcraft.api.crafting.RailcraftCraftingManager.rollingMachine.getRecipeList()
                .add(new ShapedOreRecipe(GT_Utility.copy(aResult), aRecipe));
        } catch (Throwable e) {
    ...
    public static ItemStack getRecipeOutput(boolean aUncopiedStack, ItemStack... aRecipe) {
        ...
        for (int i = 0; i < tList.size(); i++) {
            ...
            if (temp) {
                ItemStack tOutput = aUncopiedStack ? tList.get(i)
                    .getRecipeOutput()
                    : tList.get(i)
                        .getCraftingResult(aCrafting);
     ...
     * If you have multiple Mods, which add Bronze Armor for example
     */
    public static ArrayList<ItemStack> getRecipeOutputs(List<IRecipe> aList, boolean aDeleteFromList,
        ItemStack... aRecipe) {
            ...
            for (Entry<IRecipeInput, RecipeOutput> tEntry : aRecipeList.entrySet()) {
                if (tEntry.getKey()
                    ...
                    if (tEntry.getKey()
                        ...
                        ItemStack[] tList = (ItemStack[]) tEntry.getValue().items
                            .toArray(new ItemStack[tEntry.getValue().items.size()]);
     ...
     * This forces the Usage of proper Voltages (so not the transfer limits defined by the Items) unless you ignore the
     * Transfer Limit.
    ...
    public static int getCapsuleCellContainerCount(ItemStack aStack) {
        ...
        return GT_Utility.areStacksEqual(GT_Utility.getContainerForFilledItem(aStack, true), ItemList.Cell_Empty.get(1))
            || OrePrefixes.cell.contains(aStack)
            || OrePrefixes.cellPlasma.contains(aStack)
            || GT_Utility.areStacksEqual(aStack, getIC2Item("waterCell", 1, W)) ? 1 : 0;
    }
