...
public class GT_RecipeRegistrator {
     ...
     */
    public static synchronized void registerUsagesForMaterials(ItemStack aMat, String aPlate,
        boolean aRecipeReplacing) {
        if (aMat == null) return;
        ...
        for (ItemStack[] tRecipe : sShapes1) {
            ...
            if (aItemData != null && aItemData.hasValidPrefixMaterialData())
                for (ItemStack tCrafted : GT_ModHandler.getRecipeOutputs(tRecipe)) {
                    GT_OreDictUnificator.addItemData(
                        tCrafted,
                        new ItemData(aItemData.mMaterial.mMaterial, aItemData.mMaterial.mAmount * tAmount1));
                }
        }
                        ...
                        if (aItemData != null && aItemData.hasValidPrefixMaterialData())
                            GT_OreDictUnificator.addItemData(
                                ...
                                new ItemData(
                                    ...
                                    new MaterialStack(tMaterial, OrePrefixes.stick.mMaterialAmount * tAmount2)));
